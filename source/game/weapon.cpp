...
namespace game
{
    namespace weapon
    {
        ...
        void updateweapons(int curtime)
        {
            ...
            projectile::updateprojectiles(curtime); // Need to do this after the player shoots so bouncers don't end up inside player's BB next frame.
        ...
        void shoteffects(int atk, const vec& from, const vec& to, gameent* d, bool local, int id, int prevaction, bool hit)     // create visual effect from a shot
        {
            ...
            if (isvalidprojectile(projectile))
            {
                ...
                if (attackrays <= 1)
                {
                    ...
                    projectile::makeprojectile(d, from, aim, local, id, atk, projectile, attacks[atk].lifetime, attacks[atk].projspeed, attacks[atk].gravity, attacks[atk].elasticity);
                }
                else loopi(attackrays)
                {
                    projectile::makeprojectile(d, from, rays[i], local, id, atk, projectile, attacks[atk].lifetime, attacks[atk].projspeed, attacks[atk].gravity, attacks[atk].elasticity);
                }
            }
            if (validgun(gun))
            {
                ...
                {
                    projectile::spawnbouncer(d->eject, d, ejectProjectile);
                }
            }
            bool looped = false;
        ...
        int calculatedamage(int damage, gameent* target, gameent* actor, int atk, int flags)
        {
            ...
            if (!(flags & Hit_Environment))
            {
                ...
                {
                    const bool isInstagib = m_insta(mutators) && actor->type == ENT_AI && target->type == ENT_PLAYER;
                    if (attacks[atk].damage < 0 || isInstagib)
                    {
                        target->shield = 0;
            ...
            if (target->haspowerup(PU_ARMOR) || target->role == ROLE_BERSERKER)
            {
                damage /= 2;
            }
            if (!damage)
        ...
        void applyhitflags(dynent* target, const vec& from, const vec& to, const int attack, const float dist, int& flags)
        {
            ...
            else if(attacks[attack].headshotdamage)
            {
        ...
        void scanhit(vec& from, vec& to, gameent* d, int atk)
        {
            ...
            if (attacks[atk].rays > 1)
            {
                ...
                loopi(attackRays)
                {
                    if (hits[i])
                    {
                        ...
                        registerhit(o, d, rays[i], vec(to).sub(from).safenormalize(), attacks[atk].damage * numhits, atk, from.dist(to), numhits, hitFlags);
                    }
                }
            }
            else
            {
                ...
                if ((o = intersectclosest(from, to, d, attacks[atk].margin, dist, scanFlags)))
                {
                    ...
                    registerhit(o, d, to, vec(to).sub(from).safenormalize(), attacks[atk].damage, atk, from.dist(to), 1, hitFlags);
                    isHit = true;
                }
        ...
        void gibeffect(int damage, const vec& vel, gameent* d)
        {
            ...
            {
                projectile::spawnbouncer(from, d, Projectile_Gib);
            }
            if (blood)
            {
        ...
        void applyhiteffects(const int damage, gameent* target, gameent* actor, const vec& position, const int atk, const int flags, const bool isLocal)
        {
            if (!target || (!isLocal && actor == self && !(flags & Hit_Environment)))
            {
                return;
            }
            ...
            if (target->type == ENT_PLAYER || target->type == ENT_AI)
            {
                if (actor)
                {
                    ...
                    if ((isLocal || (!isLocal && hud != self)) && actor == hud && target != actor)
                    {
        ...
        void damageplayer(const int damage, gameent* target, gameent* actor, const vec& position, const int atk, const int flags, const bool isLocal)
        {
            damageentity(damage, target, actor, atk, flags, isLocal);
            applyhiteffects(damage, target, actor, position, atk, flags, isLocal);
        }

        void hitplayer(gameent* target, gameent* actor, const vec& hitposition, const vec& velocity, int damage, int attack, float dist, int rays, int flags)
        {
        ...
        void registerhit(dynent* target, gameent* actor, const vec& hitposition, const vec& velocity, int damage, int attack, float dist, int rays, int flags)
        {
            ...
            switch(target->type)
            {
                case ENT_PLAYER:
                {
                    hitplayer(f, actor, hitposition, velocity, damage, attack, dist, rays, flags);
                    break;
                }
                case ENT_AI:
                {
                ...
                case ENT_PROJECTILE:
                {
                    projectile::registerhit(target, actor, velocity, damage, attack, dist, rays, flags);
                    break;
                }
                default:
                {
        ...
        void swayinfo::update(gameent* owner)
        {
            ...
            rotationroll += swayroll * cosf(steps);
            ...
            rotationroll += cameravelocity.x * -0.3f;
