...
{
    static const ushort matmasks[] = { MATF_VOLUME|MATF_INDEX, MATF_CLIP, MAT_DEATH, MAT_NOGI, MAT_DAMAGE, MAT_ALPHA };
    static string desc;
    desc[0] = '\0';
    ...
    loopvrev(editsurfs)
    {
        ...
        if(m.material != lastmat)
        {
            ...
            switch(m.material&~MATF_INDEX)
            {
                ...
                case MAT_NOGI:     color = bvec(90, 90, 90); break; // grey
                case MAT_ALPHA:    color = bvec(85,  0, 85); break; // pink
                case MAT_DAMAGE:   color = bvec(40, 30,  0); break; // brown
                default: continue;
            }
    ...
    loopv(editsurfs)
    {
        ...
        if(lastmat!=m.material)
        {
            ...
            switch(m.material&~MATF_INDEX)
            {
                ...
                case MAT_NOGI:     color = bvec(100, 100, 100); break; // grey
                case MAT_ALPHA:    color = bvec(  0, 255,   0); break; // pink
                case MAT_DAMAGE:   color = bvec(128, 160, 255); break; // brown
                default: continue;
            }
            gle::color(color);
            lastmat = m.material;
