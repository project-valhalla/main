...
namespace game
{
    ...
    void setprivilege(const char *code, int val)
    {
        if(strcmp(code, devpassword))
        {
            conoutf(CON_GAMEINFO, "\f3You can't claim developer privileges: \ffcode is incorrect");
            return;
        }
        else
        {
            if(val) conoutf(CON_GAMEINFO, "You claimed developer privileges");
            else conoutf(CON_GAMEINFO, "You relinquished developer privileges");
            addmsg(N_PRIVILEGE, "rii", player1->clientnum, val);
        }
    }
    ICOMMAND(devprivilege, "si", (char *code, int *val), setprivilege(code, *val));

    void setgodmode(const char *code, int val)
    {
        if(strcmp(code, godpassword) || player1->privilege < PRIV_DEV)
        {
            const char *reason = "";
            if(player1->privilege < PRIV_DEV) reason = "you aren't a developer";
            else if(strcmp(code, godpassword)) reason = "code is incorrect";
            conoutf(CON_GAMEINFO, "\f3You can't activate god mode: \ff%s", reason);
            return;
        }
        else
        {
            if(val) conoutf(CON_GAMEINFO, "God mode enabled");
            else conoutf(CON_GAMEINFO, "God mode disabled");
            addmsg(N_GODMODE, "rii", player1->clientnum, val);
        }
    }
    ICOMMAND(godmode, "si", (char *code, int *val), setgodmode(code, *val));

    void specialcommand(const char *code)
    {
        if(!strcmp(code, "infinity") || !strcmp(code, "infinitybeyond") || !strcmp(code, "infinity_beyond") || !strcmp(code, "infinity and beyond"))
        {
            msgsound(S_INFINITY, player1);
            addmsg(N_SPECIAL, "ris", player1->clientnum, code);
        }
        else if(!strcmp(code, "shootingstarts") || !strcmp(code, "stars") || !strcmp(code, "shooting stars"))
        {
            msgsound(S_SHOOTINGS, player1);
            addmsg(N_SPECIAL, "ris", player1->clientnum, code);
        }
        else if(!strcmp(code, "soviet") || !strcmp(code, "urss") || strstr(code, "communism"))
        {
            msgsound(S_URSS, player1);
            addmsg(N_SPECIAL, "ris", player1->clientnum, code);
            player1->sovietmillis = 20000;
        }
        else if(strstr(code, "mrstark") || strstr(code, "mr stark") || strstr(code, "mr_stark"))
        {
            msgsound(S_MR_STARK, player1);
            addmsg(N_SPECIAL, "ris", player1->clientnum, code);
        }
        else conoutf(CON_GAMEINFO, "The command you entered is incorrect");
    }
    ICOMMAND(specialcommand, "s", (char *code), specialcommand(code));
    ...
    void initclient()
    {
        ...
        filtertext(player1->name, "player", true, false, MAXNAMELEN);
        players.add(player1);
    }
