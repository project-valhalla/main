...
- [ ] Add an 'unavailable' bool for collected items, so they can disappear immediately even if not removed by the owner yet
- [ ] Fix dropped items causing client status mismatch (e.g. ammo count)
