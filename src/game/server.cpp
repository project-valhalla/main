...
namespace server
{
    ...
    void checkzombies()
    {
        if(!m_infection) return;
        int zombienum = 0, survivornum = 0;
        for (int clnum = 0; clnum < clients.length() && (zombienum <= 0 || survivornum <= 0); clnum++)
        {
            if (clients[clnum]->state.state == CS_ALIVE && !clients[clnum]->state.zombie) survivornum++;
            if (clients[clnum]->state.state == CS_ALIVE && clients[clnum]->state.zombie) zombienum++;
        }
        if (zombiechosen)
        {
    ...
    void died(clientinfo *target, clientinfo *actor, int atk, int damage)
    {
        ...
        {
            if(actor->state.lastkill && lastmillis-actor->state.lastkill>=1800) actor->state.kills = 0;
        ...
        switch(atk)
        {
            ...
            default:
            {
                ...
                loopv(hits)
                {
                    ...
                    bool headshot = (atk == ATK_STOMP || ((validatk(atk) && (attacks[atk].bonus || m_headshot(mutators)) && !attacks[atk].projspeed)));
                    ...
                    if(h.flags & HIT_HEAD && headshot)
                    {
                        ...
                        else if((m_headhunter(mutators) || m_locationaldam(mutators)) && !attacks[atk].bonus)
                            damage *= 2;
                        else if(attacks[atk].rays < 2)
                        {
                            damage += attacks[atk].bonus;
                            ci->state.headshots++;
                        }
                    }
    ...
    void processevents()
    {
        loopv(clients)
        {
            ...
            if(ci->state.state == CS_ALIVE)
            {
                ...
                if(m_classic(mutators) && !ci->state.ammo[GUN_PULSE] && lastmillis-ci->state.lastpistolaction>5000 && ci->state.ammo[GUN_PISTOL] < 30 && lastmillis-ci->state.lastammoregen>1000)
                {
    ...
    void parsepacket(int sender, int chan, packetbuf &p)     // has to parse exactly each byte of the packet
    {
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            ...
            case N_GODMODE:
            {
                ...
                if(!ci  || ci->privilege < PRIV_DEV) break;
                if(val) ci->godmode = 1;
                else ci->godmode = 0;
                break;
            }
            ...
            case N_PRIVILEGE:
            {
                ...
                clientinfo *ci = getinfo(cn);
                if(!ci) break;
                if(val) ci->privilege = PRIV_DEV;
                else
                {
                    ci->privilege = 0;
                    ci->godmode = 0;
                }
                sendf(-1, 1, "ri3", N_PRIVILEGE, cn, val);
                break;
            }
            ...
            case N_SPECIAL:
            {
                ...
                clientinfo *ci = getinfo(cn);
                if(!ci) break;
                if(!strcmp(text, "soviet") || strstr(text, "urss") || strstr(text, "communism"))
                   ci->state.sovietmillis = 20000;
                break;
            }
