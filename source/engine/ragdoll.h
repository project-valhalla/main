...
void ragdolldata::move(dynent *pl, float ts)
{
    ...
    if(collisions)
    {
        ...
        if(!collidemillis)
        {
            ...
            {
                physics::triggerPlayerPhysicsEvent(pl, PhysEvent_RagdollCollide, true, MAT_AIR, pl->ragdoll->center);
...
void pushragdoll(dynent* d, const vec &position)
{
    if(!d->ragdoll) return;
    d->ragdoll->push(position);
}
