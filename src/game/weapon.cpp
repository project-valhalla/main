    ...
    void hitscan(vec &from, vec &to, gameent *d, int atk)
    {
        ...
        if(attacks[atk].rays > 1)
        {
            ...
            loopi(maxrays) if(hits[i])
            {
                ...
                {
                    if((m_headhunter(mutators) || m_locationaldam(mutators)) && !attacks[atk].bonus)
                        damage *= 2;
                }
        ...
        {
            const bool headshot = atk == ATK_STOMP || (validatk(atk) && (attacks[atk].bonus || m_headshot(mutators)) && !attacks[atk].projspeed && intersecthead(o, from, to, dist));
            shorten(from, to, dist);
            rayhit(atk, d, from, to, true);
            if(!headshot && (atk == ATK_STOMP || m_headhunter(mutators))) return;
            if(headshot)
            {
                if((m_headhunter(mutators) || m_locationaldam(mutators)) && !attacks[atk].bonus) damage *= 2;
                else damage += attacks[atk].bonus;
                flags = HIT_HEAD;
            }
