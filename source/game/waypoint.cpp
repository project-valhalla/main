...
namespace ai
{
    ...
    int getweight(const vec &o)
    {
        ...
        if(isliquidmaterial(posmat&MATF_VOLUME)) weight *= 5;
        if(dist >= 0)
        {
            ...
            else if(isliquidmaterial(trgmat & MATF_VOLUME)) weight *= 2;
        }
        return weight;
    }
    ...
    bool getwaypointfile(const char *mname, char *wptname)
    {
        ...
        nformatstring(wptname, MAXSTRLEN, "data/map/%s.wpt", mname);
        path(wptname);
        return true;
    }
